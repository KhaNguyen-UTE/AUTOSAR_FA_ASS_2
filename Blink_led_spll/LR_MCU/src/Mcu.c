...
#define MCU_SW_PATCH_VERSION_C              4
...
{   
    ...
    if ((Std_ReturnType)E_OK == (Std_ReturnType)Mcu_HLDChecksEntry(MCU_INITRAMSECTION_ID))
    {
        if ((Std_ReturnType)E_OK == (Std_ReturnType)Mcu_CheckInitRamSection(RamSection))
        {
            ...
            for (RamCounter = (Mcu_RamIndexType)0UL; RamCounter < RamCounterLimit; RamCounter++)
            {
                 switch (pRamConfigPtr->Mcu_RamWriteSize)
                {
                    case (Mcu_RamWriteSizeType)1U:
                        ...
                        /* @violates @ref Mcu_c_REF_8 Violates MISRA 2004 Advisory Rule 11.4 , A cast should not be performed*/
                         ( *((uint8*) (pRamConfigPtr->Mcu_pu8RamBaseAddr + RamCounter * (pRamConfigPtr->Mcu_RamWriteSize)))) = (uint8)pRamConfigPtr->Mcu_u64RamDefaultValue; 
                        break;

                    case (Mcu_RamWriteSizeType)2U:
                        /* @violates @ref Mcu_c_REF_9 Violates MISRA 2004 Advisory Rule 1.2: Undefined behaviour */
                        /* @violates @ref Mcu_c_REF_8 Violates MISRA 2004 Advisory Rule 11.4 , A cast should not be performed*/
                         ( *((uint16*) (pRamConfigPtr->Mcu_pu8RamBaseAddr + RamCounter * (pRamConfigPtr->Mcu_RamWriteSize)))) = (uint16)pRamConfigPtr->Mcu_u64RamDefaultValue; 
                        break;

                    case (Mcu_RamWriteSizeType)4U:
                        /* @violates @ref Mcu_c_REF_9 Violates MISRA 2004 Advisory Rule 1.2: Undefined behaviour */
                        /* @violates @ref Mcu_c_REF_8 Violates MISRA 2004 Advisory Rule 11.4 , A cast should not be performed*/
                         ( *((uint32*) (pRamConfigPtr->Mcu_pu8RamBaseAddr + RamCounter * (pRamConfigPtr->Mcu_RamWriteSize)))) = (uint32)pRamConfigPtr->Mcu_u64RamDefaultValue; 
                        break;

                    case (Mcu_RamWriteSizeType)8U:
                        /* @violates @ref Mcu_c_REF_9 Violates MISRA 2004 Advisory Rule 1.2: Undefined behaviour */
                        /* @violates @ref Mcu_c_REF_8 Violates MISRA 2004 Advisory Rule 11.4 , A cast should not be performed*/
                         ( *((uint64*) (pRamConfigPtr->Mcu_pu8RamBaseAddr + RamCounter * (pRamConfigPtr->Mcu_RamWriteSize)))) = (uint64)pRamConfigPtr->Mcu_u64RamDefaultValue; 
                        break;
...
{Std_ReturnType, MCU_CODE) Mcu_GetPeripheralState(VAR( Mcu_PeripheralIdType, AUTOMATIC) McuPeriphId)
{
    /* Result of the operation. */
...
*                   value shall evidence the current running mode, clock settings and activation,
*                   etc.
*
...
/**
* @brief            This function disable or enable SleepOnExit.
* @details          Disable or enable Sleep on exit when returning from Handler mode to Thread mode.
*
...
* @api
*
* @implements Mcu_CM4_SleepOnExit_Activity
*
*/
/** @violates @ref Mcu_c_REF_6 Violates MISRA 2004 Required Rule 8.10, global declaration of
* function
*/
FUNC(void, MCU_CODE) Mcu_CM4_SleepOnExit(VAR(Mcu_SleepOnExitType, AUTOMATIC) SleepOnExit)
{
#if ( MCU_VALIDATE_GLOBAL_CALL == STD_ON )
    if((Std_ReturnType)E_OK == (Std_ReturnType)Mcu_HLDChecksEntry(MCU_SLEEPONEXIT_ID))
    {
            ...
            Mcu_Ipw_CM4_SleepOnExit(SleepOnExit);
