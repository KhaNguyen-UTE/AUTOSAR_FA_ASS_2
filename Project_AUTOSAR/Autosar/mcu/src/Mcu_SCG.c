...
#define MCU_SCG_SW_PATCH_VERSION_C               4
...
/*==================================================================================================
                                       GLOBAL VARIABLES
==================================================================================================*/
#define SCG_SCS_SPLL_RunMode    SCG_SCS_SPLL_U32 | \
                                SCG_DIVCORE_U32((uint32)1U) | \
                                SCG_DIVBUS_U32((uint32)1U) | \
                                SCG_DIVSLOW_U32((uint32)1U)

#define SCG_SCS_SPLL_HSRunMode  SCG_SCS_SPLL_U32 | \
                                SCG_DIVCORE_U32((uint32)1U) | \
                                SCG_DIVBUS_U32((uint32)1U) | \
                                SCG_DIVSLOW_U32((uint32)1U)
...
/***********************************created by HaoTV13********************************************/
/**
* @brief            This function will configure the system clock for all modes using PLL clock
*/
FUNC( void, MCU_CODE) Mcu_SCG_ActivePllClock(VAR( void, AUTOMATIC))
{
    VAR(uint32, AUTOMATIC) u32TimeOut = 0U;
    VAR(uint32, AUTOMATIC) u32RegValue = 0U;
    
    REG_WRITE32(SCG_RCCR_ADDR32, SCG_SCS_SPLL_RunMode);
    /* Wait for switching system clock source done. */
    u32RegValue = REG_READ32(SCG_RCCR_ADDR32) & SCG_RCCR_SCS_MASK32; 
    u32TimeOut = MCU_TIMEOUT_LOOPS;    
    do
    ...
    while ((SCG_SCS_SPLL_RunMode != u32RegValue) && ((uint32)0x00U < u32TimeOut));

#ifdef MCU_HSRUN_MODE_NOT_SUPPORT
#if (MCU_HSRUN_MODE_NOT_SUPPORT == STD_ON)
    /* do nothing */
#else
    REG_WRITE32(SCG_HCCR_ADDR32, SCG_SCS_SPLL_HSRunMode);
    /* Wait for switching system clock source done. */
    u32RegValue = REG_READ32(SCG_HCCR_ADDR32) & SCG_HCCR_SCS_MASK32;
    u32TimeOut = MCU_TIMEOUT_LOOPS;    
    do
    ...
    while ((SCG_SCS_SPLL_HSRunMode  != u32RegValue) && ((uint32)0x00U < u32TimeOut));
#endif
#endif
}
/***********************************created by HaoTV13********************************************/
...
*/
FUNC( void, MCU_CODE) Mcu_SCG_DropSystemClockToTrustedClock(VAR( void, AUTOMATIC))
{
    VAR(uint32, AUTOMATIC) u32TimeOut = MCU_TIMEOUT_LOOPS;
    VAR(uint32, AUTOMATIC) u32RegValue = 0U;
    
    /* Check SIRC is enabling or not before setup a default config */
    /** @violates @ref Mcu_SCG_c_REF_4 Conversion from pointer to integer */
    /** @violates @ref Mcu_SCG_c_REF_5 The cast is used to access memory mapped registers.*/
    u32RegValue = REG_READ32(SCG_SIRCCSR_ADDR32) & SCG_SIRCCSR_SIRCEN_MASK32;
    if (SCG_SIRCCSR_SIRCEN_MASK32 != u32RegValue)
    {        
    ...
    /* do nothing */
#else
    /* In HSRUN mode, we will set FIRC clock to selects the clock source generating the system clock */
    /* Check FIRC is enabling or not before setup a default config */
    /** @violates @ref Mcu_SCG_c_REF_4 Conversion from pointer to integer */
    /** @violates @ref Mcu_SCG_c_REF_5 The cast is used to access memory mapped registers.*/
    u32RegValue = REG_READ32(SCG_FIRCCSR_ADDR32) & SCG_FIRCCSR_FIRCEN_MASK32;
    /*Wait for Valid bit done */
    u32TimeOut = MCU_TIMEOUT_LOOPS;
    if (SCG_FIRCCSR_FIRCEN_MASK32 != u32RegValue)
    {
        /* write some default values for FIRC registers */
        /** @violates @ref Mcu_SCG_c_REF_4 Conversion from pointer to integer */
        /** @violates @ref Mcu_SCG_c_REF_5 The cast is used to access memory mapped registers.*/
        REG_WRITE32(SCG_FIRCCSR_ADDR32,SCG_FIRCCSR_LK_UNLOCK_U32);
        /** @violates @ref Mcu_SCG_c_REF_4 Conversion from pointer to integer */
        /** @violates @ref Mcu_SCG_c_REF_5 The cast is used to access memory mapped registers.*/
        REG_WRITE32(SCG_FIRCCFG_ADDR32,SCG_FIRCCFG_48MHZ_U32);
        /** @violates @ref Mcu_SCG_c_REF_4 Conversion from pointer to integer */
        /** @violates @ref Mcu_SCG_c_REF_5 The cast is used to access memory mapped registers.*/
        REG_WRITE32(SCG_FIRCDIV_ADDR32,SCG_FIRCDIV_FIRCDIV2_U32(1U) |  SCG_FIRCDIV_FIRCDIV1_U32(1U));
        /** @violates @ref Mcu_SCG_c_REF_4 Conversion from pointer to integer */
        /** @violates @ref Mcu_SCG_c_REF_5 The cast is used to access memory mapped registers.*/
        REG_BIT_SET32(SCG_FIRCCSR_ADDR32, SCG_FIRCCSR_FIRCEN_MASK32);
        do
        {
            u32TimeOut--;
            /** @violates @ref Mcu_SCG_c_REF_4 Conversion from int to pointer */
    ...
    /** @violates @ref Mcu_SCG_c_REF_4 Conversion from pointer to integer */
    /** @violates @ref Mcu_SCG_c_REF_5 The cast is used to access memory mapped registers.*/
    REG_RMW32(SCG_HCCR_ADDR32, SCG_HCCR_SCS_MASK32, SCG_SCS_FIRC_U32);
    /* Wait for switching system clock source done. */
#ifdef MCU_E10777_IPV_SCG_0001
#if (MCU_E10777_IPV_SCG_0001 == STD_ON)
    /* Support for e10777 errata: SCG_RCCR[SCS] and SCG_HCCR[SCS] should be read twice by software to get correctly value */
    ...
    while ((SCG_SCS_FIRC_U32 != u32RegValue) && ((uint32)0x00U < u32TimeOut));
    /* Raise a error report if the timeout is expired */
    if ((uint32)0x0U == u32TimeOut)
    {
#if (MCU_DISABLE_DEM_REPORT_ERROR_STATUS == STD_OFF)
...
#if (MCU_NO_PLL == STD_OFF)
/**
* @brief            This function will clear Clock Monitor Reset bit and System OSC Clock Monitor bit
*
* @details          This function will clear Clock Monitor Reset bit and System OSC Clock Monitor bit before
*                   initialize SOSC module to avoid reset due to LOL event.
*
*/
/**
* @violates @ref Mcu_SCG_c_REF_6 Violates MISRA 2004 Required Rule 8.10, global declaration of function
*/
FUNC( void, MCU_CODE) Mcu_SCG_PrepareSpllBeforeSoscInit(VAR( void, AUTOMATIC))
{
    VAR(uint32, AUTOMATIC) u32RegValue = 0U;
...
/**
* @brief    This function returns the value of System PLL Clock Monitor and System PLL Clock Monitor Reset Enable
*
* @details  This function returns the current configurations of System PLL Clock Monitor and System PLL Clock Monitor Reset Enable
*/
/**
* @violates @ref Mcu_SCG_c_REF_6 Violates MISRA 2004 Required Rule 8.10, global declaration of function
*/
FUNC(uint32, MCU_CODE) Mcu_SCG_GetCurrentPLLClockMonitorConfig(VAR(void, AUTOMATIC))
{
    VAR( uint32, AUTOMATIC) u32PLLClockMonitorConfig;
    /* get RCM_SRIE and mask agains what i need */
    /** @violates @ref Mcu_SCG_c_REF_4 Conversion from int to pointer */
    /** @violates @ref Mcu_SCG_c_REF_5 The cast is used to access memory mapped registers.*/
    u32PLLClockMonitorConfig = REG_READ32(SCG_SPLLCSR_ADDR32);
    u32PLLClockMonitorConfig &= (SCG_SPLLCSR_SPLLCM_MASK32 | SCG_SPLLCSR_SPLLCMRE_MASK32);

    return ((uint32)u32PLLClockMonitorConfig);
}

/**
* @brief    This function Restore PLL Clock Monitor Config
*/
/**
* @brief    This function Restore PLL Clock Monitor Config
*
* @details  This function Restore the clock monitor configurations which were disabled when initialize sosc
*/
/**
* @violates @ref Mcu_SCG_c_REF_6 Violates MISRA 2004 Required Rule 8.10, global declaration of function
*/
FUNC(void, MCU_CODE) Mcu_SCG_RestorePLLClockMonitorConfig(VAR(uint32, AUTOMATIC) u32PLLClockMonitorConfig)
{
    /** @violates @ref Mcu_SCG_c_REF_4 Conversion from int to pointer */
    /** @violates @ref Mcu_SCG_c_REF_5 The cast is used to access memory mapped registers.*/
    REG_RMW32(SCG_SPLLCSR_ADDR32, SCG_SPLLCSR_SPLLCM_MASK32 | SCG_SPLLCSR_SPLLCMRE_MASK32, u32PLLClockMonitorConfig);
}
...
*
* @details          This function will disable all clock monitors
*
*/
/**
* @violates @ref Mcu_SCG_c_REF_6 Violates MISRA 2004 Required Rule 8.10, global declaration of function
...
FUNC( void, MCU_CODE) Mcu_SCG_DisableFircClock(VAR( void, AUTOMATIC))
{
    ...
    /** @violates @ref Mcu_SCG_c_REF_5 The cast is used to access memory mapped registers.*/
    REG_BIT_CLEAR32(SCG_FIRCCSR_ADDR32, SCG_FIRCCSR_FIRCEN_MASK32);
    /* wait for FIRCEN bit clear. */
    do
    {
        u32TimeOut--;
        /** @violates @ref Mcu_SCG_c_REF_4 Conversion from pointer to integer */
        /** @violates @ref Mcu_SCG_c_REF_5 The cast is used to access memory mapped registers.*/
        u32RegValue = REG_BIT_GET32(SCG_FIRCCSR_ADDR32, SCG_FIRCCSR_FIRCEN_MASK32);
    } while ((SCG_FIRCCSR_FIRCEN_MASK32 == u32RegValue) && ((uint32)0x00U < u32TimeOut));

    if ((uint32)0x00 != u32TimeOut)
    {
        u32TimeOut = MCU_TIMEOUT_LOOPS;
        /* wait for valid bit done. */
        do
        {
            ...
            /** @violates @ref Mcu_SCG_c_REF_4 Conversion from int to pointer */
            /** @violates @ref Mcu_SCG_c_REF_5 The cast is used to access memory mapped registers.*/
            u32RegValue = REG_BIT_GET32(SCG_FIRCCSR_ADDR32, SCG_FIRCCSR_FIRCVLD_MASK32);
        }
        while ((SCG_FIRCCSR_FIRCVLD_MASK32 == u32RegValue) && ((uint32)0x00U < u32TimeOut));
    }
