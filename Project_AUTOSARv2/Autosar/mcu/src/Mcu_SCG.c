...
/*
Run Core Clock Frequency:   24MHz
Run System Clock Frequency: 24MHz
Run Bus Clock Frequency:    24MHz  
Run Flash Clock Frequency:  12MHz  
*/                                            
...
/*
HSRun Core Clock Frequency:   96MHz
HSRun System Clock Frequency: 96MHz
HSRun Bus Clock Frequency:    48MHz  
HSRun Flash Clock Frequency:  24MHz  
*/
...
FUNC( void, MCU_CODE) Mcu_SCG_SircInit(P2CONST(Mcu_SCG_ClockConfigType, AUTOMATIC, MCU_APPL_CONST) pConfigPtr)
{
    ...
    if ((REG_READ32(SCG_SIRCCSR_ADDR32) & SCG_SIRCCSR_SIRCEN_MASK32) == SCG_SIRCCSR_SIRCEN_MASK32)
    {
        ...
        if ( (uint32)0x0U == u32TimeOut)
        {
            (void) Det_ReportError((uint16)MCU_MODULE_ID, MCU_INSTANCE_ID, MCU_INITCLOCK_ID, MCU_E_INIT_SIRC_FAILED);
        }
    }
}
