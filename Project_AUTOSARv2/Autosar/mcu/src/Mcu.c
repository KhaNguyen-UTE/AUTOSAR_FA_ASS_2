...
*                       MCU_DISTRIBUTEPLLCLOCK_OK, MCU_DISTRIBUTEPLLCLOCK_NOT_OK.
*
* @implements     Mcu_GetPllStatusType_enumeration
*
*/
typedef enum
{
    MCU_DISTRIBUTEPLLCLOCK_OK = 0x1U,   /**< @brief PLL is locked and GetPLL was called */
    MCU_DISTRIBUTEPLLCLOCK_NOT_OK = 0x0U,   /**< @brief PLL is unlocked or GetPLL was not called */
} Mcu_GetPllStatusType;

static VAR(Mcu_GetPllStatusType, AUTOMATIC) Call_PllStatus = MCU_DISTRIBUTEPLLCLOCK_NOT_OK; //Created by HAOTV13
/*************************************************************************
**************************Created by HAOTV13******************************
**************************************************************************/
...
FUNC(void, MCU_CODE) Mcu_Init( P2CONST( Mcu_ConfigType, AUTOMATIC, MCU_APPL_CONST) pConfigPtr)
{
    ...
    VAR(uint32, AUTOMATIC) u32NoConfigs; /* Variable to save Mcu_Init config*/

    VAR(Std_ReturnType, AUTOMATIC ) checkStatus; /* Variable to check valid status while init*/
...
FUNC( Std_ReturnType, MCU_CODE) Mcu_DistributePllClock(VAR( void, AUTOMATIC))
{
    ...
    {
        /* Check if the PLL is locked and if GetPLLStatus was called. */
        if(Call_PllStatus == MCU_DISTRIBUTEPLLCLOCK_OK)
        {
...
FUNC( Mcu_PllStatusType, MCU_CODE) Mcu_GetPllStatus( VAR( void, AUTOMATIC))
{
    ...
    /* if the PLL is not used in the configuration return MCU_PLL_STATUS_UNDEFINED */
        ...
        if (ePllStatus == MCU_PLL_LOCKED) 
            {
                Call_PllStatus = MCU_DISTRIBUTEPLLCLOCK_OK;
            }
