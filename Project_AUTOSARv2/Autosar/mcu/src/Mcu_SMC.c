...
#define MCU_SMC_SW_PATCH_VERSION_C               4
...
/* Check if source file and Mcu_CortexM4.h are from the same vendor */
#if (MCU_SMC_VENDOR_ID_C != MCU_CORTEXM4_VENDOR_ID)
    #error "Mcu_SMC.C and Mcu_CortexM4.h have different vendor ids"
#endif
/* Check if source file and Mcu_CortexM4.h are of the same Autosar version */
#if ((MCU_SMC_AR_RELEASE_MAJOR_VERSION_C != MCU_CORTEXM4_AR_RELEASE_MAJOR_VERSION) || \
     (MCU_SMC_AR_RELEASE_MINOR_VERSION_C != MCU_CORTEXM4_AR_RELEASE_MINOR_VERSION) || \
     (MCU_SMC_AR_RELEASE_REVISION_VERSION_C != MCU_CORTEXM4_AR_RELEASE_REVISION_VERSION) \
    )
    #error "AutoSar Version Numbers of Mcu_SMC.c and Mcu_CortexM4.h are different"
#endif
/*Check if source file and Mcu_CortexM4.h are of the same Software version */
#if ((MCU_SMC_SW_MAJOR_VERSION_C != MCU_CORTEXM4_SW_MAJOR_VERSION) || \
     (MCU_SMC_SW_MINOR_VERSION_C != MCU_CORTEXM4_SW_MINOR_VERSION) || \
     (MCU_SMC_SW_PATCH_VERSION_C != MCU_CORTEXM4_SW_PATCH_VERSION) \
    )
    #error "Software Version Numbers of Mcu_SMC.c and Mcu_CortexM4.h are different"
#endif
...
static FUNC( Std_ReturnType, MCU_CODE) Mcu_SMC_Get_VLPSA_Status(void);
#endif
...
*               Function return E_OK if enter VLPS was successfully, E_NOT_OK if enter VLPS was Aborted.
*
* @param[in]    none
*
...
static FUNC( Std_ReturnType, MCU_CODE) Mcu_SMC_Get_VLPSA_Status(void)
{
    VAR(Std_ReturnType, AUTOMATIC) AbortStatus = (Std_ReturnType)E_NOT_OK;
    ...
    u32RegValue = REG_READ32(SMC_PMCTRL_ADDR32) & SMC_PMCTRL_STOPA_MASK32;
    if((uint8)0x00U == u32RegValue)
    {
        AbortStatus = (Std_ReturnType)E_OK;
    }
    else
    {
        AbortStatus = (Std_ReturnType)E_NOT_OK;
    }
    return AbortStatus;
}
#endif
...
FUNC( void, MCU_CODE) Mcu_SMC_AllowedModesConfig(P2CONST(Mcu_SMC_ConfigType, AUTOMATIC, MCU_APPL_CONST) pConfigPtr)
{
    ...
    /* Only allowed normal run mode*/
    #ifdef MCU_HSRUN_VLPR_MODE_NOT_SUPPORT
    REG_WRITE32(SMC_PMPROT_ADDR32, (pConfigPtr->u32NotAllowedModes & SMC_PMPROT_RWBITS_MASK32));
    #endif
}
    ...
    switch (ePowerMode)
    {
        ...
        case (MCU_VLPS_MODE):
        {
            if ((SMC_PMSTAT_RUN_U32 == eCurrentPowerMode) || (SMC_PMSTAT_VLPR_U32 == eCurrentPowerMode))
            {
                ...
                EXECUTE_WAIT();
#ifdef MCU_VLPSA_NOTIFICATION
                /** @violates @ref Mcu_SMC_c_REF_6 MISRA 2004 Conversion from integer to pointer */
                if(E_NOT_OK == Mcu_SMC_Get_VLPSA_Status())
                {
